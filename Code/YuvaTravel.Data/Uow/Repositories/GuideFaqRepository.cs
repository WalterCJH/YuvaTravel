...
{
    public class GuideFaqRepository : BaseRepository<GuideFaq>, IGuideFaqRepository
    {
        public GuideFaqRepository(YuvaTravelDbContext db) : base(db) { }

        public IQueryable<GuideFaq> All(bool isANT = true)
        {
        ...
        public IQueryable<GuideFaq> IncludeAll(bool isANT = true)
        {
        ...
        public async Task<GuideFaq> FindAsync(Guid? id, bool isANT = true)
        {
            return await IncludeAll(isANT).FirstOrDefaultAsync(p => p.GuideFaqId == id);
        }

        public IQueryable<GuideFaq> Search(GuideFaqFilter filter)
        {
            var data = IncludeAll();
            if (filter.GuideId.HasValue)
                data = data.Where(p => p.GuideId == filter.GuideId.Value);
            if (!string.IsNullOrEmpty(filter.Keyword))
                data = data.Where(p => p.Question.Contains(filter.Keyword));
